...
public class SysRedisConfig {
	...
	@Autowired
	private SysRedisProperties properties;
	
	@Bean("sysRedisHandler")
	public RedisHandler sysRedisHandler() {
		return new RedisHandler(sysObjectRedisTemplate());
	}
	
	@Bean("sysObjectRedisTemplate")
	public ObjectRedisTemplate sysObjectRedisTemplate() {
		  return new ObjectRedisTemplate(sysRedisConnectionFactory());
	}
	
	private LettuceConnectionFactory sysRedisConnectionFactory() {
